...
public:
    ...
    {
        if (startsWith(request.getURI(),"/author") || startsWith(request.getURI(),"/search")) {
            return new AuthorHandler(_format);
        } else if (startsWith(request.getURI(), "/article") || startsWith(request.getURI(), "/accept_article")) {
            return new ArticleHandler(_format);
        }
